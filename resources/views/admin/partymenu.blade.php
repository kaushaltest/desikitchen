...
<main class="app-main">
    ...
    <div class="app-content-header">
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-sm-6">
                    <h3 class="mb-0">Party Menu</h3>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-end">
                        <li class="breadcrumb-item"><a href="#">Home</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Party menu</li>
                    </ol>
                </div>
            </div>
            <!--end::Row-->
                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body table-responsive">
                            ...
                            <table id="dt_partymenu" class="table table-hover text-nowrap">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>Image</th>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <select class="form-select bg-gray" id="price_unit" name="price_unit">
...
<script>
    $(document).ready(function() {
        ...
        var table = $("#dt_partymenu").DataTable({
            ajax: {
                url: '{{route("admin.get-party-menu")}}', // Replace with your server endpoint
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return (data.price_per_kg) ? "<span><i class='fa fa-usd'></i>" + data.price_per_kg + " / <small>per kg</small></span>" : "<span><i class='fa fa-usd'></i>" + data.price_per_qty + "/ <small>per qty</small></span>";
                    },
                },
                {
                    data: 'is_active',
        ...
        $('#dt_partymenu').on('click', '.btn-menu-edit', function(e) {
            e.preventDefault();
            $('#form_add_edit_menu').validate().resetForm();
            $('#form_add_edit_menu')[0].reset();
            ...
            $('#txt_price').val((rowData.price_per_kg) ? rowData.price_per_kg : rowData.price_per_qty);
            $('input[name="rbt_is_active"][value="' + rowData.is_active + '"]').prop('checked', true);

            // Show the modal
            $('#model_add_edit_menu').modal('toggle');
        });
        $('#dt_partymenu').on('click', '.btn-menu-delete', function(e) {
            const row = $(this).closest('tr');
            const rowData = table.row(row).data();
            let confirmation = confirm("Are you sure want to delete this menu ?");
            if (confirmation) {
                $.ajax({
                    url: "{{ route('admin.delete-partymenu') }}", // Change this to your server endpoint
                    type: 'POST',
                    headers: {
        ...
        $('#form_add_edit_menu').validate({
            ...
            submitHandler: function(form, event) {
                ...
                $.ajax({
                    url: "{{ route('admin.add-update-party-menu') }}", // Change this to your server endpoint
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': "{{ csrf_token() }}"
                    },
